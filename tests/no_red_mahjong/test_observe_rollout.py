...
in ``round_mode='half'``, which is ``NoRedMahjong``'s own default and the mode both
bugs required.
"""
...
def test_is_discard_node_matches_the_legal_actions(rollout):
    for state, obs in rollout:
        ...
        expected = bool(legal[:34].any() or legal[68])  # discards, or tsumogiri
...
def test_last_draw_is_never_another_players_draw(rollout):
    ...
    for state, obs in rollout:
        ...
        assert bool(obs["is_discard_node"]) or bool(state.legal_action_mask[70])
        hand = np.asarray(state.players.hand[state.current_player])
        assert hand[shown] > 0, (
...
def test_last_draw_is_masked_during_a_robbing_kan_window():
    ...
    state = default_state()
    hand = state.players.hand.at[responder, 5].set(1)
    legal = state.players.legal_action_mask.at[responder, 71].set(True)  # RON
    legal = legal.at[responder, 77].set(True)  # PASS
    state = state.replace(
        current_player=jnp.int8(responder),
        legal_action_mask=legal[responder],
        players=state.players.replace(hand=hand, legal_action_mask=legal),
...
def test_target_is_present_whenever_a_call_is_legal(rollout):
    ...
    call_actions = [71, 72, 73, 74, 75, 76]  # RON, PON, OPEN_KAN, CHI_L/M/R
    for state, obs in rollout:
        legal = np.asarray(state.legal_action_mask)
...
def test_melds_match_the_raw_state_and_counts(rollout):
    ...
    for state, obs in rollout:
        ...
        # No red fives here, so ``Meld.target`` IS the tile; red needs ``target_tile``
        # to re-attach the red bit.
        np.testing.assert_array_equal(melds[1], np.asarray(Meld.target(raw)).astype(np.int8))
...
def test_meld_actions_are_valid_call_ids(rollout):
    for _, obs in rollout:
        ...
        # 34-67 self-kan, 72-73 pon/open_kan, 74-76 chi variants.
        assert bool((((actions >= 34) & (actions <= 67)) | ((actions >= 72) & (actions <= 76))).all()), actions
...
def _expected_tiles_seen(state) -> np.ndarray:
    ...
    for seat in range(4):
        for slot in range(24):
            ...
            seen[t] += 1  # no red fives: the river tile IS the tile type
    ...
    for seat in range(4):
        for slot in range(4):
            ...
            if 74 <= action <= 76:  # chi variants
                low = target - (action - 74)  # CHI_L/M/R are consecutive here
                for off in range(3):
                    seen[low + off] += 1
            elif action == 73 or 34 <= action <= 67:  # open / closed / added kan
                seen[target] += 4
            else:  # pon / pon_red
                seen[target] += 3
...
def test_tiles_seen_includes_own_hand_and_dora(rollout):
    ...
    for ind in np.asarray(state.round_state.dora_indicators):
        if int(ind) >= 0:
            assert int(np.asarray(obs["tiles_seen"])[int(ind)]) >= 1
...
def test_observation_module_imports_standalone():
    ...
    died at ``from mahjax.no_red_mahjong.observation import _observe_dict``).
    """
    import subprocess
    import sys

    result = subprocess.run(
        [sys.executable, "-c", "import mahjax.no_red_mahjong.observation as m; assert m._observe_dict"],
        capture_output=True,
        text=True,
    )
